...
const main = async () => {
  ...
  try {
    ...
    const autoResultHasRelations = autoEvents.at(-1)?.data.every((row: Record<string, unknown>) => 'courier' in row && 'stops' in row)
    assert.equal(autoResultHasRelations, true)
    record('SSE auto-result-has-relations', autoResultHasRelations)
    ...
    const paginatedPageMeta = paginatedAutoEvents.find((event) => event.type === 'pageMeta')
    assert.deepEqual(paginatedPageMeta, { type: 'pageMeta', total: 6, hasMore: true })
    const paginatedResult = paginatedAutoEvents.at(-1)?.data
    const paginatedResultHasCourier = Array.isArray(paginatedResult?.data)
      && paginatedResult.data.every((row: Record<string, unknown>) => 'courier' in row)
    assert.equal(paginatedResultHasCourier, true)
    record('SSE paginated-auto-types', paginatedAutoEvents.map((event) => event.type))
    record('SSE paginated-auto-page-meta', paginatedPageMeta)
    record('SSE paginated-auto-result-shape', Object.keys(paginatedResult ?? {}).sort())
    record('SSE paginated-auto-result-has-courier', paginatedResultHasCourier)
    ...
    assert.deepEqual(fallbackErrorEvents.map((event) => event.type), ['error'])
    ...
    const fallbackJson = await readJson(await get(base, unsupportedPath, { 'x-api-variant': 'single' }))
    const fallbackSingleData = fallbackSingleEvents.at(-1)?.data
    assert.deepEqual(fallbackSingleEvents.map((event) => event.type), ['result'])
    assert.equal(fallbackJson.status, 200)
    const fallbackSingleMatchesJson = JSON.stringify(fallbackSingleData) === JSON.stringify(fallbackJson.body)
    const fallbackSingleHasStops = Array.isArray(fallbackSingleData)
      && fallbackSingleData.every((row: Record<string, unknown>) => 'stops' in row)
    assert.equal(fallbackSingleMatchesJson, true)
    assert.equal(fallbackSingleHasStops, true)
    record('SSE fallback-single', fallbackSingleEvents.map((event) => event.type))
    record('SSE fallback-single-matches-json', fallbackSingleMatchesJson)
    record('SSE fallback-single-has-stops', fallbackSingleHasStops)
