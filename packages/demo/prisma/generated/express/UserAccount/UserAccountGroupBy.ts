...
interface GroupByRequest extends Request {
  prisma: PrismaClient
  query: Partial<Prisma.UserAccountGroupByArgs> & ParsedQs
...
export async function UserAccountGroupBy(
  req: GroupByRequest,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    // @ts-ignore
    const result = await req.prisma.userAccount.groupBy(req.query)
