...
interface CountRequest extends Request {
  prisma: PrismaClient
  query: Partial<Prisma.UserAccountCountArgs> & ParsedQs
...
export async function UserAccountCount(
  req: CountRequest,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    const result = await req.prisma.userAccount.count(
      req.query as Prisma.UserAccountCountArgs,
    )
