...
interface UpsertRequest extends Request {
  prisma: PrismaClient
  body: Prisma.UserAccountUpsertArgs
...
export type UpsertMiddleware = RequestHandler<
  ParamsDictionary,
  any,
  Prisma.UserAccountUpsertArgs,
  Record<string, any>
>

export async function UserAccountUpsert(
  req: UpsertRequest,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    const data = await req.prisma.userAccount.upsert(req.body)
