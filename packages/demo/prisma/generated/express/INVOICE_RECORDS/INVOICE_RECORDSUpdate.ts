...
interface UpdateRequest extends Request {
  prisma: PrismaClient
  body: Prisma.INVOICE_RECORDSUpdateArgs
...
export type UpdateMiddleware = RequestHandler<
  ParamsDictionary,
  any,
  Prisma.INVOICE_RECORDSUpdateArgs,
  Record<string, any>
>

export async function INVOICE_RECORDSUpdate(
  req: UpdateRequest,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    const data = await req.prisma.iNVOICE_RECORDS.update(req.body)
