...
export type ModelRelationDirection =
  | 'parentOwnsFk'
  | 'childOwnsFk'
  | 'implicitM2M'
...
function walk(
  ...
  if (depth >= ctx.maxDepth) {
    return {
      unsupportedReason: 'nested depth reached maxDepth=' + ctx.maxDepth,
    }
  }
  ...
  if (hasWhereLikeRelationReference(parentArgs, model)) {
    return {
      unsupportedReason:
        'relation used in where/orderBy/cursor is not supported in MVP',
    }
  }

  const projection = isPlainObject(select)
    ? select
    : isPlainObject(include)
      ? include
      : null
  if (!projection) {
    return {}
  ...
  if (relationBranches.length === 0) {
    return {
      projectionAfterStrip: isSelectMode ? updatedProjection : undefined,
    }
  }
  ...
  for (const branch of relationBranches) {
    ...
    if (branch.value !== true && !isPlainObject(branch.value)) {
      return {
        unsupportedReason:
          'invalid relation projection value for ' +
          branch.name +
    ...
    for (const linkField of relation.parentLinkFields) {
      if (localOmit && localOmit[linkField] === true) {
        ...
        return {
          unsupportedReason:
            'required parent link field omitted: ' + where + linkField,
        }
      }
    }
    ...
    const relationArgs: Record<string, unknown> =
      branch.value === true ? {} : branch.value

    const allowedArgs = relation.isList
      ? ALLOWED_TO_MANY_ARGS
      : ALLOWED_TO_ONE_ARGS
    ...
    if (hasWhereLikeRelationReference(relationArgs, targetModel)) {
      return {
        unsupportedReason:
          'nested relation used in where/orderBy/cursor for ' +
          relation.name +
...
export function planAutoInclude(
  input: AutoIncludePlannerInput,
): AutoIncludePlan {
  ...
  if (result.unsupportedReason) {
    return {
      ...
      internalFieldPaths: [],
      unsupportedReason:
        'auto-progressive fallback: ' + result.unsupportedReason,
    }
  }
  ...
  if (result.projectionAfterStrip) {
    ...
    const stripped = Object.fromEntries(
      Object.entries(input.args.include).filter(
        ([k]) => !rootModel.relations[k],
      ),
    )
