...
async function request(
  ...
  if (target === 'hono') {
    ...
    const router = module.ArticleRouter as (
      config: Record<string, unknown>,
    ) => Hono
  ...
  try {
    const result = await fetch(
      `http://127.0.0.1:${(server.address() as AddressInfo).port}/?take=2`,
    )
    return { status: result.status, body: await result.json() }
  } finally {
...
for (const target of ['express', 'fastify', 'hono'] as const) {
  describe(`${target} generated override`, () => {
    it.each([true, false])(
      'wrap=%s keeps typed input/context and exposes only the guarded operation',
      async (wrap) => {
        const recorded = recordingDelegate()
        const override: RuntimeOperationOverride = async ({
          input,
          args,
          context,
          prisma,
          ...
          return wrap
            ? await core()
            : [{ id: 'replacement', site_id: 'tenant-a' }]
        }
        const result = await request(
          target,
          {
            ...
            disableOpenApi: true,
            queryBuilder: false,
            resolveContext: () => ({ actor: 'editor' }),
            findMany: { shape, override },
          },
          recorded.prisma,
        )
        ...
        expect(result.body).toEqual(
          wrap ? [] : [{ id: 'replacement', site_id: 'tenant-a' }],
        )
      },
    )
        ...
        target === 'express'
          ...
          : target === 'fastify'
            ? async (
                _req: object,
                reply: {
                  code: (status: number) => { send: (body: object) => void }
                },
              ) => {
                events.push('access')
                reply.code(403).send({ error: 'not_permitted' })
              }
            : () => {
                events.push('access')
                return new Response('{"error":"not_permitted"}', {
                  status: 403,
                })
              }
      const result = await request(
      ...
      const result = await request(
        target,
        {
          ...
          findMany: {
            variants: {
              'tenant/:role': {
                shape: (context: { tenant: string }) => {
                  order.push('guard')
                  return { where: { site_id: context.tenant } }
                },
              },
            },
            override: () => {
      ...
      for (const terminal of [false, true]) {
        ...
        const hook = (name: string) =>
          target === 'express'
            ? (((_req, res, next) => {
                order.push(name)
                if (terminal && name === 'before')
                  res.status(409).json({ stopped: true })
                else next()
              }) satisfies RequestHandler)
            : target === 'fastify'
              ? async (
                  ...
                  order.push(name)
                  if (terminal && name === 'before')
                    reply.code(409).send({ stopped: true })
                }
              : () => {
                  order.push(name)
                  if (terminal && name === 'before')
          ...
          target,
          {
            ...
            findMany: {
              ...
              variants: {
                reader: {
                  shape,
                  before: [hook('variant-before')],
                  after: [hook('variant-after')],
                },
              },
              override: async ({ core }: { core: () => Promise<unknown> }) => {
                order.push('override')
                return core()
              },
            },
